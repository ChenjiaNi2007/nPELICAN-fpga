...
typedef ap_fixed<24, 5, AP_RND_CONV, AP_SAT> relu_t;  // act_layer (QuantReLU) (signed): scale=2^-19 (1.907348633e-06), bits=24, k=19
typedef ap_fixed<24, 1, AP_RND_CONV, AP_SAT> t0_t;  // post_agg 2->0 (signed): scale=2^-23 (1.192092896e-07), bits=24, k=23
typedef ap_fixed<24, 2, AP_RND_CONV, AP_SAT> out_t;  // output_quant (signed): scale=2^-22 (2.384185791e-07), bits=24, k=22
typedef ap_fixed<24, 1, AP_RND_CONV, AP_SAT> w1_gen_t;  // 2->2 weights (signed): scale=2^-23 (1.192092896e-07), bits=24, k=23
typedef ap_fixed<24, 2, AP_RND_CONV, AP_SAT> w2_gen_t;  // 2->0 weights (signed): scale=2^-22 (2.384185791e-07), bits=24, k=22
...
typedef ap_fixed<26, 2, AP_RND_CONV, AP_SAT> bias_t_gen;  // b1,b1_diag,b2 (float); |bias|max=1.17512 (I=2), F=24
typedef ap_fixed<43, 10, AP_RND_CONV, AP_SAT> bn_t_gen;  // BN mean/scale/beta (float); |c|max=355.871 (I=10), F=33
...
typedef ap_fixed<30, 6, AP_RND_CONV, AP_SAT> bn1out_t;  // batch1 = BN1(dots); |batch1|<=29.4 (I=6), F=24
...
typedef ap_fixed<38, 14, AP_RND_CONV, AP_SAT> tr_t;  // Tr = BN2(relu); |Tr|<=5694.5 (I=14, vs t0_t I=1 which would saturate), F=24
typedef ap_fixed<47, 23> acc0_t;     // R full sum of tr_t (I(tr_t)+H2)
typedef ap_fixed<43, 19> acc0row_t;  // trace, sum of tr_t (I(tr_t)+H1)

// ---- MAC temporaries: I = I(weight)+I(operand)+ceil(log2(#terms)), W = I+B ----
typedef ap_fixed<51, 10> mac2_t;     // 2->2 dense: 6 w1*t2 products + b1 + b1_diag = 8 terms
typedef ap_fixed<51, 6> mac0_t;     // 2->0 dense: 2*NHIDDEN w2*t0 products + b2 = 5 terms
