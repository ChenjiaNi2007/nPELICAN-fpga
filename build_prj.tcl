...
proc remove_recursive_log_wave {} {
    ...
    set filename ${project_name}_prj/solution/sim/verilog/${project_name}.tcl
...
if {$opt(cosim)} {
    ...
    add_files -tb ${project_name}_tb.cpp -cflags "-std=c++0x -DRTL_SIM"
    set time_start [clock clicks -milliseconds]
    ...
    set old_pwd [pwd]
    cd ${project_name}_prj/solution/sim/verilog/
    source run_sim.tcl
    cd $old_pwd
    ...
    puts "INFO:"
    if {[string equal "$backend" "vivadoaccelerator"]} {
        puts [read [open ${project_name}_prj/solution/sim/report/${project_name}_axi_cosim.rpt r]]
    } else {
        puts [read [open ${project_name}_prj/solution/sim/report/${project_name}_cosim.rpt r]]
    }
...
if {$opt(vsynth)} {
    ...
    if {[file exist ${project_name}_prj/solution/syn/vhdl]} {
        set time_start [clock clicks -milliseconds]
