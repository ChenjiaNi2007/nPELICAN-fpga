...
### Lever 8 — BN1 past the 2→2 aggregation + T0 ROM  ✓ IMPLEMENTED 2026-09-23 (branch `bn1-fold`; csynth/vsynth owed, remote)

**What.** BN1 (`batch1 = (s·d + β')·m`, `s = γ/σ`, `β' = β − μ·s`) is consumed in exactly
two ways: (A) per element as the basis op `T0 = Q_t2(batch1)`, and (B) linearly by the
2→2 sums `jmass = Σ_ij batch1 / N̄²`, `jdotp[j] = Σ_i batch1_ij / N̄`. Both are rewritten so
`batch1` is never materialized:

- **(A) T0 via ROM.** `d` is a `dot_t` with `DOT_W` bits, so `T0(d)` is a function of the
  raw `DOT_W`-bit code: a `2^DOT_W`-entry `t2_t` table `bn1_t0_rom`, **generated by the
  loader by running the rebuilt Brevitas model's own `MessageNet` (BN1) and
  `post_agg_quant` on every code** — faithful by construction (incl. PyTorch's float32 BN
  arithmetic and eps). Firmware: `T0 = m ? bn1_t0_rom[dots.range(DOT_W−1,0)] : 0`,
  upper triangle + mirror, ROM completely partitioned (each lookup = a mux of constants,
  ≲1 LUT6 per output bit). The mask select is required: `ROM[code(0)] = Q(β') ≠ 0`, while
  PyTorch zeroes masked `batch1` before quantizing.
- **(B) Sums from RAW dots, BN1 applied once per aggregate** (same algebra as Lever 4):
  ```
  Σ_ij batch1    = s·Σ_ij d·m        + β'·ncount²
  Σ_i  batch1_ij = (s·Σ_i d_ij·m_ij  + β'·ncount)·m_jj
  jmass    = t2(dsum·bn1_s_all + ncount²·bn1_b_all)          bn1_s_all = s/N̄², bn1_b_all = β'/N̄²
  jdotp[j] = t2((rowsum[j]·bn1_s_row + ncount·bn1_b_row)·m_jj) bn1_s_row = s/N̄,  bn1_b_row = β'/N̄
  ```
  The four folded constants are computed in double by the loader from the UNROUNDED BN
  constants (not the `--bn-frac-bits`-capped `bn_t_gen` literal) and typed `bn1fold_t`
  with `F = t2_F + H2 + dot_mag + 4` (constant rounding error × largest raw sum
  `2^(dot_mag+H2)` stays under 2⁻⁴ of half a t2 LSB). The raw accumulators
  `accdot2_t = <dot_W+9, dot_I+9>` / `accdotrow_t = <dot_W+5, dot_I+5>` hold exact
  integers on the dot grid; every product/sum is exact in HLS's promoted types, so the
  **only rounding is the single t2_t cast** (AP_RND_CONV, AP_SAT). `ncount`/`ncount2` are
  hoisted and shared with the Lever-4 BN2 collapse.
- **Fallback** (no ROM: `DOT_W > 10`, or the float-reference build): arithmetic T0 with a
  single rounding straight to `t2_t` (no `bn1out_t` intermediate); (B) is unchanged.

**Why it is invariant-safe.** BN1 is still an explicit affine (loader-supplied constants)
applied BEFORE the post-agg quantizer — nothing is folded into the dense weights; the
N-dependent additive term is explicit via the runtime `ncount`/`ncount²`. Normalize-late is
kept (raw sum → one scaled rounding). Masking: padded T0 entries are exactly 0 (select),
padded summands are masked explicitly (`·nobjmask`, a 1-bit select). PIPELINE II=1 and all
complete partitions untouched; frozen weight order untouched (new arrays are appended to
`weights.h`). It is **more faithful to PyTorch**: the 484 intermediate `bn1out_t`
roundings (F=6 at the 6-bit operating point) are gone, and PyTorch also sums the
unrounded `batch1`.
...
### Lever 9 — Winograd inner-product dot (2 multiplies per pair)  ✓ IMPLEMENTED 2026-09-25 (branch `bn1-fold`, `winograd=1`; csynth/vsynth owed)

**The algebra.** Winograd's inner-product identity: for a·b = Σ_k a_k b_k with 4 terms,
pair up the terms and precompute one correction per row and per column,

    a·b = (a0 + b1)(a1 + b0) + (a2 + b3)(a3 + b2) − (a0 a1 + a2 a3) − (b0 b1 + b2 b3).

With a_i = (E, px, py, pz)_i and b_j = (E, −px, −py, −pz)_j (the metric folded into b):

    xi[i]  =  E_i px_i + py_i pz_i                         (per particle)
    eta[j] = −E_j px_j + py_j pz_j                         (per particle)
    d_ij   = (E_i − px_j)(px_i + E_j) + (py_i − pz_j)(pz_i − py_j) − xi[i] − eta[j]

Expanding the two pair products gives E_iE_j − px_i px_j − py_i py_j − pz_i pz_j plus
exactly xi[i] + eta[j], which the corrections remove. xi and eta are the sum and the
difference of the SAME two per-particle products (E·px, py·pz), so the per-particle cost is
2 multiplies, not 4.

**Multiply count.** 2 × 253 (upper-triangle pairs incl. diagonal) + 2 × 22 (particles incl.
beams) = **550 vs 1,012** for `dot4`. With `const_beams=1` the 43 beam pairs keep `dot4`
(which folds to E_j ∓ pz_j, no multiply; Winograd would NOT fold there because its pair
products mix particle j's own components, cancelled only through eta[j]): 2 × 210 + 2 × 20 =
**460 vs 840**. The firmware does this carve-out automatically when both flags are set.

**Exactness (why the gate is bit-identical, not just close).** Nothing is narrowed before the
single `dot_t` cast. For `input_t = ap_fixed<W,I>`: each ± of two momenta is exact in
`<W+1,I+1>`; each product of two such sums is exact in `<2W+2,2I+2>`; the two-product sum in
`<2W+3,2I+3>`; xi/eta are exact in `dotxi_t = ap_fixed<2W+1,2I+1>` (generated by the loader;
`<25,23>` for this pmu-12 checkpoint); the two subtractions need at most `<2W+5,2I+5>`. The
whole right-hand side of `dot4_winograd` is ONE expression, so HLS keeps the promoted exact
width, and the one `(dot_t)` cast (AP_RND_CONV, AP_SAT) rounds the identical value `dot4`
rounds. Masking is inherited: a padded particle's p1 row is all zero, so xi = eta = 0 and
every term of its pair expression is 0. Beam rows are never masked; the identity is exact for
any input_t values, so runtime (boosted) beams are covered too. Verified: local gate 200/200
exact, max|Δ| 0 on GOLDEN and DOTS-LEVEL for monolith / split / split=2 (also with
`const_beams`), `golden_fw_results.log` and `fw_stage_dump.txt` byte-identical to the
committed record (incl. the `dots:` line); plus 4M random operand quadruples (incl. ±full-scale
saturation codes) per width at `<12,11>`, `<18,12>`, `<36,12>` and negative-F `<10,12>`:
0 mismatches of the exact value and of the `dot_t` cast.

**Why it dodges the 759 `mac_mulsub` floor (7j).** 7j showed three of the four products per
pair sit on DSP48 for their accumulate structure, not their width, so NARROWING can only ever
touch 1 in 4 dot DSPs. Lever 9 reduces the NUMBER of products, not their width: each remaining
product still maps to one DSP48E2 with its pre-adder and post-adder used — the floor was 3
per pair, now the whole dot is 2 per pair.

**Operand-width caveat.** The pair operands are sums of two momenta, W+1 bits. A DSP48E2 is
27×18: one factor can come out of the 27-bit pre-adder (A±D), the other goes on the 18-bit B
port, which has no pre-adder. So one DSP per product needs W+1 ≤ 18, i.e. **momenta ≤ 17
bits**. Fine at pmu-12 (13-bit sums); at the old 18-bit operating point (Lever 2) each product
would need 2 DSPs and the lever saves nothing. The products are 2W+2 bits, within the 48-bit P.

**Expected adder cost (the price).** Per pair: four (W+1)-bit sums, two absorbable by the DSP
pre-adders (E_i − px_j, py_i − pz_j), two in fabric (px_i + E_j, pz_i − py_j, the B-port
operands); plus the xi/eta subtractions (2W+1 to 2W+5 bits), which the DSP post-adder can
absorb only if HLS routes the per-particle corrections into the C port / cascade (store −xi,
−eta per particle; DSP1 = M1 + C, DSP2 = M2 + PCIN + C via the W mux) — whether HLS does is
unknown. Per particle: 22 × (one sum + one difference) of 2W-bit products, negligible. At W=12
that is ~26 LUT/pair best case (2 fabric 13-bit adders, ~6.6k LUT for 253 pairs) to ~110
LUT/pair worst case (all four sums + two ~27-bit corrections in fabric, ~27k LUT), i.e.
**~14–60 LUT per DSP saved** (462 DSPs). The top of that range is the same order as the 7j
"−1 DSP for +28 LUT" wrong-direction trade, so this lever pays only where DSP is the binding
budget (a smaller device / SLR, or the DSP headroom is spent elsewhere); decide on vsynth.

**Stacking.** Composes with Lever 5 (`const_beams`, see the carve-out above) and with Lever 3
(II relaxation: fewer products to time-multiplex; at II=2 with sharing the 550 halve again).
Does NOT apply under Lever 7 block-FP (per-particle mantissa/exponent operands would need
per-pair exponent alignment of every sum): `NPELICAN_WINOGRAD_DOT` + `NPELICAN_BLOCK_FP` is a
compile-time `#error`.

**Build flag.** `build_prj.tcl winograd=1` → `-DNPELICAN_WINOGRAD_DOT` (default 0; use `reset=1`
when toggling); locally `./build_local.sh [split|split2] -DNPELICAN_WINOGRAD_DOT`. Firmware:
`dot4_winograd()` (declared in `nPELICAN.h`, defined in `nPELICAN.cpp` / `nPELICAN_split.cpp`),
xi/eta computed in the dot stage (`DotXi:` loop) after p1 prep. `dotxi_t` is generated in
`types_generated.h` (`NPELICAN_DOTXI_T_GENERATED`), with a hand fallback in `nPELICAN.h`
derived from `input_t` (`<36,12>` → `<73,25>`) and `double` in `types_float.h`.

**csynth/vsynth owed** (remote): confirm 550 dot multipliers on DSP (expect ~1,066 − 462 ≈ 604
DSP total on this checkpoint), read the per-pair adder LUT from the csynth Expression table,
and judge the LUT/DSP exchange on vsynth.
...
- **Narrowing the dot multiplier operands to save DSP** — 3 of the 4 products per pair
  ...
  saved. This closes block-FP AND uniform narrowing as DSP levers. (Reducing the NUMBER
  of products is a different lever and is open: Lever 9, Winograd dot, 2 per pair.)
...
8. **Lever 9 (Winograd dot, `winograd=1`)** — implemented 2026-09-25, bit-identical gate.
   Next: csynth + vsynth on the Lever 8 FINAL checkpoint (expect dot DSP 1,012 → 550, and
   read the per-pair adder LUT to get the LUT/DSP exchange rate). Only worth keeping if DSP
   is the binding budget; stacks with `const_beams` (460 vs 840) and with Lever 3 (II
   relaxation). Uniform momenta only (≤ 17 bits); not with block-FP.
